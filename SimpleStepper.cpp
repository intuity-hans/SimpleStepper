...
SimpleStepper::SimpleStepper(uint8_t dirpin0, uint8_t steppin0, uint8_t dirpin1, uint8_t steppin1, uint8_t dirpin2, uint8_t steppin2)
{
    if (!firstInstance)
    {
        firstInstance = this;
...
bool SimpleStepper::step(uint8_t identifier, long steps, uint8_t direction, long pulse)
{
    if (this->isStepping(identifier))
    {
        return false;
